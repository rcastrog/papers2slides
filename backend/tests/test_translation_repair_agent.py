"""Smoke test for TranslationRepairAgent."""
        ...
        return json.dumps(
            {
                ...
                "target_ids": ["deck"],
                "changes_made": ["Aligned translated terminology"],
                "unresolved_risks": [],
...
class TranslationRepairAgentSmokeTest(unittest.TestCase):
    def test_run_returns_validated_repair_result(self) -> None:
        ...
        agent = TranslationRepairAgent(llm_client=llm_client, prompt_loader=prompt_loader)

        result = agent.run({"audit_findings": []})

        self.assertIsInstance(result, RepairResult)
        self.assertEqual(result.target_ids[0], "deck")
