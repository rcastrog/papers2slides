"""Smoke test for VisualRepairAgent."""
        ...
        return json.dumps(
            {
                ...
                "target_ids": ["GV01"],
                "changes_made": ["Added provenance warning"],
                "unresolved_risks": [],
...
class VisualRepairAgentSmokeTest(unittest.TestCase):
    def test_run_returns_validated_repair_result(self) -> None:
        ...
        agent = VisualRepairAgent(llm_client=llm_client, prompt_loader=prompt_loader)

        result = agent.run({"audit_findings": []})

        self.assertIsInstance(result, RepairResult)
        self.assertEqual(result.target_ids[0], "GV01")
